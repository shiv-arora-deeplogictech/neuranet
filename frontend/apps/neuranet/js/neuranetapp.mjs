...
async function _createdata(data) {   
    ...
    const loginresponse = session.get(APP_CONSTANTS.LOGIN_RESPONSE), appsAllowed = [...(loginresponse?.apps||[])],
        ...
        const isDeclaredAppAdmin =  (appsAllowed.some(app => app.admins?.includes(loginresponse.id)) || appsAllowed.some(app=> app.is_user_appadmin === true));
    if (isAdmin || isDeclaredAppAdmin) appsAllowed.push({id: AI_WORKSHOP_VIEW, interface: {type: AI_WORKSHOP_VIEW,
